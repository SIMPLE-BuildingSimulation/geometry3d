...
impl Segment3D {
    ...
    pub fn contains(&self, input:&Segment3D)-> Result<bool,String> {
        ...
        let tiny = 1E-9;
        if self.length() < tiny {
            let msg = format!("Trying to check whether a segment is contained in a zero-length segment");
            return Err(msg);
        }
        ...
        return Ok(alpha >=0. && alpha <=1. && beta >= 0. && beta <= 1.);
    }
    ...
    pub fn intersect(&self, input: &Segment3D, output: & mut Point3D)-> bool{
        ...
        let t_b : f64;
        let det : f64;
        let tiny = 1E-10 as f64;

        // Check for intersection.
        if normal.z().abs() > tiny {
            det = a.y()*b.x() - a.x()*b.y();
        ...
        if t_a >= 0.0 && t_a <= 1.0 && t_b >= 0.0 && t_b <= 1.0 {
            let x = self.start.x() + t_a * a.x();
            let y = self.start.y() + t_a * a.y();
            let z = self.start.z() + t_a * a.z();

            *output = Point3D::new(x,y,z);
