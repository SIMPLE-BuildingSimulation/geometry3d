...
impl Triangle3D {
    pub fn new(a:Point3D, b:Point3D, c:Point3D, i:usize) -> Result<Triangle3D,String>{

        // check that points are not the same
        if a.compare(b) || a.compare(c) || b.compare(c) {
            let msg = format!("Trying to build a Triangle3D with two equal points");
            return Err(msg);
        }

        // Check that they are not collinear
        if a.is_collinear(b, c).unwrap(){
            let msg = format!("Trying to build a Triangle3D from collinear points");
            return Err(msg);
        }
        ...
        let mut t = Triangle3D {
            ...
            s0:Segment3D::new(a,b),
            s1:Segment3D::new(b,c),
            s2:Segment3D::new(c,a),
    ...
    pub fn set_neighbour(&mut self, edge:usize, i:usize)->Result<(),String>{
        match edge {
            ...
            1 => self.n1=i as i32,
            2 => self.n2=i as i32,
            _ => return Err(format!("Trying to set neighbour with index '{}'... it is out of bounds",i)),
        }
        Ok(())
    }
    ...
    pub fn neighbour(&self, edge:usize)->Result<i32,String>{
        match edge {
            ...
            1 => Ok(self.n1),
            2 => Ok(self.n2),
            _ => return Err(format!("Trying to get neighbour with index '{}'... it is out of bounds",edge)),
        }        
    }
    ...
    fn set_area(&mut self){
        if self.area < 0. {
            ...
            self.area = ((c + b + a)*((c + b + a) / 2. - a)*((c + b + a) / 2. - b)*((c + b + a) / 2. - c) / 2.).sqrt();
        }        
    }
    ...
    fn set_circumcenter(&mut self){
        ...
        let a_center = ((ab_cross_ac.cross(ab))*ac_sq_length + ac.cross(ab_cross_ac) * ab_sq_length) / (2. * ab_x_ac_sq_length);
    ...
    pub fn vertex(&self, i:usize)-> Result<Point3D,String> {
        match i {
            ...
            1 => return Ok(self.v1),
            2 => return Ok(self.v2),
            _ => return Err(format!("Trying to get vertex with index '{}'... it is out of bounds",i)),
        }
    }
    ...
    pub fn segment(&self, i:usize)->Result<Segment3D,String>{
        match i {
            ...
            1 => return Ok(self.s1),
            2 => return Ok(self.s2),
            _ => return Err(format!("Trying to get Segment with index '{}'... it is out of bounds",i)),
        }
    }
    ...
    pub fn constrain(&mut self, i:usize)-> Result<(),String>{
        match i {
            0 => {self.c0 = true; return Ok(());},
            1 => {self.c1 = true; return Ok(());},
            2 => {self.c2 = true; return Ok(());},
            _ => return Err(format!("Trying to constrain a vertex with index '{}'... it is out of bounds",i)),
        }
    }
    ...
    pub fn is_constrained(&self, i:usize)-> Result<bool,String>{
        match i {
            ...
            1 => Ok(self.c1),
            2 => Ok(self.c2),
            _ => return Err(format!("Checking for a constrained vertex with index '{}'... it is out of bounds",i)),
        }
    }
}
