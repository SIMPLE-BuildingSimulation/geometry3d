...
impl Loop3D {
    ...
    pub fn push(&mut self, point: Point3D) -> Result<usize, &'static str> {
        ...
        if !self.normal.is_zero() {
            ...
            let r = self.is_coplanar(point).unwrap();
            if !r {
    ...
    pub fn vertex(&self, i: usize) -> Result<(Point3D, bool), &'static str> {
        if i < self.vertices.len() {
            ...
            Ok((v.position, v.valid))
        } else {
            Err("Trying to get vertex out of Loop's bounds")
        }
    }
    ...
    pub fn invalidate_vertex(&mut self, i: usize) -> Result<(), String> {
        if i < self.vertices.len() {
            ...
            Err("Trying to get vertex out of Loop's bounds".to_string())
        }
    }
    ...
    pub fn is_coplanar(&self, p: Point3D) -> Result<bool, String> {
        ...
        Ok((self.normal * d).abs() < f64::EPSILON)
    }
    ...
    pub fn test_point(&self, point: Point3D) -> Result<bool, String> {
        ...
        */
        ...
        let mut previous_x = xp * (first_point - point);
        ...
        for i in 1..(n_vertex + 1) {
            ...
            let v = vertex.position - point;

            // determine x and y
            let pt_y = v * yp;
            let pt_x = v * xp;

            if pt_y.abs() < f64::EPSILON && previous_y.abs() < f64::EPSILON {
                continue; // no rotation! Moving along the X-axis
            } else if previous_y * pt_y < -f64::EPSILON {
                // Crosses the axis
                let r = previous_x + previous_y * (pt_x - previous_x) / (previous_y - pt_y);
                if r > 0.0 {
                    // if crosses on the correct side
                    if previous_y < 0.0 {
                        // crosses positive
                        w_number += 1.0;
                    } else {
            ...
            } else if previous_y.abs() < f64::EPSILON && previous_x > 0.0 {
            ...
            } else if pt_y.abs() < f64::EPSILON && pt_x > 0.0 {
