...
impl Vector3D{
    ...
    pub fn is_parallel(&self, v: Vector3D) -> bool{
        ...
        if v.x.abs() > tiny {
            ...
            return check_other(k, self.y, v.y) && check_other(k, self.z, v.z);

        } else if v.y.abs() > tiny {
            // Let's try anchoring at Y
            k = self.y / v.y;

            return check_other(k, self.x, v.x) && check_other(k, self.z, v.z);
        } else {
            ...
            k = self.z / v.z;
            //}
            return check_other(k, self.x, v.x) && check_other(k, self.y, v.y);
        }
...
mod testing{
    ...
    fn test_scale(){
        ...
        let x=1.2;let y=5.22; let z=9.123;
        let s=4.123;
