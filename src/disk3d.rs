...
impl Disk3D {
    ...
    pub fn new(centre: Point3D, normal: Vector3D, radius: Float)->Self{
        Self::new_detailed(centre, normal, radius, 0., normal.get_perpendicular().unwrap(), 360., None)
    }

    pub fn new_detailed(centre: Point3D, normal: Vector3D, radius: Float, inner_radius: Float, phi_zero: Vector3D, phi_max: Float, transform: Option<Rc<Transform>>) -> Self {
        // Normalize
        let normal = normal.get_normalized();
        // Check phi_zero
        if normal.is_parallel(phi_zero){
            panic!("when creating a 'disk': normal ({}) is parallel to phi_zero ({})", normal, phi_zero);
        }
        ...
        if radius <= inner_radius {
            panic!("when creating a 'disk': radius ({}) is smaller or equal than inner_radius ({})", radius, inner_radius);
        }
        if radius < 0. {
            panic!("when creating a 'disk': radius ({}) is negative", radius);
        }
        if inner_radius < 0. {
            panic!("when creating a 'disk': inner_radius ({}) is negative", inner_radius);
        }
    ...
    pub fn basic_intersection(&self, ray: &Ray3D, _o_error: Point3D, _d_error: Point3D)->Option<(Point3D, Float)>{
        ...
        if r_squared > self.radius * self.radius || r_squared < self.inner_radius * self.inner_radius {
            return None
        }
    ...
    pub fn intersection_info(&self, ray: &Ray3D, phit: Point3D, phi: Float)-> Option<IntersectionInfo> {
        let r = phit - self.centre;
        let rhit = r.length();
        // Calculate (u,v)
        ...
        let dpdu = self.phi_zero * (-rhit_sin_phi) + zxn * (rhit_cos_phi/rhit * (self.inner_radius - self.radius));
        let dpdv = self.phi_zero * (-rhit_cos_phi) + zxn * (rhit_sin_phi/rhit * (self.radius - self.inner_radius));
...
impl Intersect for Disk3D {
    ...
    fn simple_intersect_local_ray(&self, ray: &Ray3D, o_error: Point3D, d_error: Point3D) -> Option<Point3D> {        
